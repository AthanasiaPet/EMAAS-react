...
function About() {
    return (
        <section id="about" className="bg-[#ebd2c3] px-6 py-24">
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-start">

                {/* Left side - Text */}
                <div>
                    <p className="text-[11px] uppercase tracking-[0.14em] text-[#1a1a1a] mb-4">About EMAAS</p>
                    <div className="w-12 h-0.5 bg-[#cc3300] mb-6"></div>
                ...
                <div className="flex flex-col gap-5">
                    {cards.map((card, index) => (
                        <div key={index} className="bg-white/20 border border-[#d4a898] border-l-4 border-l-[#cc3300] p-7 hover:shadow-md transition-shadow">
                            <h3 className="font-serif text-lg font-semibold text-[#1a1a1a] mb-3">{card.title}</h3>
                            <p className="text-[#1a1a1a] text-sm leading-relaxed">{card.text}</p>
                        </div>
                    ))}
                </div>
